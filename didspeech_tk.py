...
from tk_gui.gui.mainwindow import MainWindow
from tk_gui.gui.frame import SelectFileFrame, OptionsFrame, OutputFrame
from tk_gui.utils import misc
from tk_gui.utils.misc import print_d, tb_replace, browse, time_2_ms, check_time_format
from tk_gui.multithread.multithread import *

# Global var
THREADS = 4	# threads number

def start_parse(file, start, end, out=None, chunk_size=30000):  
...
if __name__ == "__main__":
	...
	#------------------ Create Windows --------------------------------------------
	window = MainWindow(title="Didspeech - Trascribe audio file")
	window.create_window()

	#------------------ Frame select file -----------------------------------------
	# frame top left
	f_select_file = SelectFileFrame(master=window, x=0.0, y=0.06, h=0.3, w=0.45) 
	...
	#------------------ Frame options ---------------------------------------------
	# frame top right
	f_options = OptionsFrame(master=window, x=0.5, y=0.05, h=0.3, w=0.45)
	...
	e_start = tk.Entry(f_options, bg="green")
	e_start.insert(0, "00:00:00")
	e_start.grid(row=0, column=1)
	e_end = tk.Entry(f_options, bg="red")
	...
	#------------------ Frame output ----------------------------------------------
	f_out = OutputFrame(master=window, x=0.025, y=0.27, h=0.7, w=0.95)

	# output text box 
	tb_out = scrolledtext.ScrolledText(f_out, undo=True, height=15, bg="white")
